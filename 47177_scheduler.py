...
class Scheduler:
    ...
    def _exec_local(self, ttype, payload):
        if ttype=="code": return self.code.run(payload)
        if ttype=="video": return self.video.run(payload)
        if ttype=="voice": return self.voice.run(payload)
        if ttype=="automation": return self.auto.run(payload)
        ...
        while not self._stop.is_set():
            ...
            for tid, t in tasks.items():
                ...
                try:
                    ...
                    gpu = False
                    if ttype == "video" and Settings.ROLE == "control" and wants_diffusion(payload.get("prompt","")):
                        res = self.cluster.dispatch("video", dict(payload, task_id=tid))
                        gpu = True
                    else:
