...
def load_properties(
        f, interestingprops: "list[str | tuple[str, str]] | None" = None):
    fetch(f)
    props = {}
    re1 = re.compile(r"^\s*([0-9A-F]+)\s*;\s*(\w+)(?:\s*;\s*(\w+))?")
    re2 = re.compile(
        r"^\s*([0-9A-F]+)\.\.([0-9A-F]+)\s*;\s*(\w+)(?:\s*;\s*(\w+))?")
...
def emit_property_module(f, mod, tbl, emit: "list[str | tuple[str, str]]"):
    ...
    for cat_str, cat in cats:
        ...
        f.write(
            "        super::util::bsearch_range_table(c, %s_table)\n" %
            cat_str)
        f.write("    }\n\n")
    f.write("}\n\n")
...
""" % UNICODE_VERSION)
        ...
        derived = load_properties(
            "DerivedCoreProperties.txt", [
                "Alphabetic", ("InCB", "Consonant"), ("InCB", "Extend"), ("InCB", "Linker")])

        emit_util_mod(rf)
        for (
            name, cat, pfuns) in (
            "general_category", gencats, ["N"]), ("derived_property", derived, [
                "Alphabetic", ("InCB", "Extend")]):
    ...
    matches!(c,""")
        ...
        for chars in grapheme_table:
            ...
            last = chars[1]
        emit_break_module(
            rf, grapheme_table, list(
                grapheme_cats.keys()), "grapheme")
        rf.write("\n")
        ...
        # For efficiency, we could still merge the two tables and produce an
        # ALetterEP state
        emoji_table = [(x, y, "Extended_Pictographic")
                       for (x, y) in emoji_props["Extended_Pictographic"]]
        emit_break_module(rf, emoji_table, ["Extended_Pictographic"], "emoji")

        sentence_cats = load_properties("auxiliary/SentenceBreakProperty.txt")
        sentence_table = []
        for cat in sentence_cats:
            sentence_table.extend([(x, y, cat)
                                  for (x, y) in sentence_cats[cat]])
        sentence_table.sort(key=lambda w: w[0])
        emit_break_module(
            rf, sentence_table, list(
                sentence_cats.keys()), "sentence")
