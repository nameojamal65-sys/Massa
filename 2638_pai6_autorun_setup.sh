#!/bin/bash
# -----------------------------
# PAI6 Full Auto-Run Setup
# -----------------------------

ROOT="$HOME/PAI6_UltimateClosure"
REPORT_DIR="$ROOT/reports"
AUTOSCRIPT="$HOME/pai6_deep_fusion_auto.sh"
BOOTFILE="$HOME/.termux/boot.sh"

mkdir -p "$ROOT"
mkdir -p "$REPORT_DIR"
mkdir -p "$HOME/.termux"

# --- 1. إنشاء سكربت التشغيل التلقائي ---
cat > "$AUTOSCRIPT" << 'EOF'
#!/bin/bash
ROOT="$HOME/PAI6_UltimateClosure"
REPORT_DIR="$ROOT/reports"
...
echo "✅ Setup Complete!"
echo "🖥 PAI6 will run automatically on Termux launch."
echo "📄 Reports will be in: $REPORT_DIR"
