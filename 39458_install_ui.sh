...
<head>
  ...
  <style>
    ...
    input, select, textarea, button {
      ...
      border:1px solid #23364d; background:#0b0f14; color:#e7eef8;
      outline: none;
    }
...
<div class="card">
  ...
  <div class="muted">
    ...
    جرّب من هنا داخل الواجهة: افتح <code>/docs</code> أو استعمل curl على
    ...
    try {
      ...
      tbody.innerHTML = files.map(f =>
        `<tr>
          ...
          <td>
            <button class="linkbtn" onclick="deleteFile(${f.id})">Delete</button>
          </td>
        </tr>`
      ).join("");
    } catch(e) {
...
echo "✅ Patching app/main.py to include UI router..."
MAIN="app/main.py"
if [ ! -f "$MAIN" ]; then
  echo "❌ ما لقيت $MAIN"
  exit 1
...
# أضف import إذا مش موجود
if ! grep -q "from app.web.router import router as ui_router" "$MAIN"; then
  # جرّب نضيفه بعد آخر import يبدأ بـ from app
  sed -i '1i\from app.web.router import router as ui_router\n' "$MAIN"
fi

# أضف include_router إذا مش موجود
if ! grep -q "app.include_router(ui_router)" "$MAIN"; then
  # ضيفه بعد أول include_router موجود أو بعد إنشاء app
  if grep -q "include_router" "$MAIN"; then
    # بعد أول include_router
    awk '
      BEGIN{done=0}
      {print}
      (!done && $0 ~ /app\.include_router\(/){print "app.include_router(ui_router)"; done=1}
    ...
    awk '
      BEGIN{done=0}
      {print}
      (!done && $0 ~ /^app *= *FastAPI/){print "app.include_router(ui_router)"; done=1}
    ' "$MAIN" > "$MAIN.tmp" && mv "$MAIN.tmp" "$MAIN"
  fi
fi
...
echo "2) uvicorn app.main:app --host 127.0.0.1 --port 9000"
echo "Then open: http://127.0.0.1:9000/ui/login"
