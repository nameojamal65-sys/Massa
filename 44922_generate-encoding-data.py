...
if (not os.path.isfile("../encoding/encodings.json")
    ) or (not os.path.isfile("../encoding/indexes.json")):
...
if not os.path.isfile("../codepage/src/lib.rs"):
    sys.stderr.write(
        "This script also writes the generated parts of the codepage crate and needs a clone of https://github.com/hsivonen/codepage next to the encoding_rs directory.\n")
    sys.exit(-1)
...
def read_non_generated(path):
    ...
    if generated_begin_index < 0:
        sys.stderr.write(
            "Can't find generated code start marker in %s. Exiting.\n" %
            path)
        sys.exit(-1)
    generated_end_index = full.find(generated_end)
    if generated_end_index < 0:
        sys.stderr.write(
            "Can't find generated code end marker in %s. Exiting.\n" %
            path)
        sys.exit(-1)
...
for name in preferred:
    ...
    if is_single_byte(name):
        (run_bmp_offset, run_byte_offset,
         run_length) = longest_run_for_single_byte(name)
        variant = "SingleByte(&data::SINGLE_BYTE_DATA.%s, 0x%04X, %d, %d)" % (to_snake_name(
            u"iso-8859-8" if name == u"ISO-8859-8-I" else name),
            run_bmp_offset,
            run_byte_offset,
            run_length)
    else:
    ...
    label_file.write(
        '''/// The initializer for the [%s](static.%s.html) encoding.
///
...
for label in labels:
    label_test_file.write(
        '''assert_eq!(Encoding::for_label(b"%s"), Some(%s));\n''' %
        (label.label, to_constant_name(
            label.preferred)))
...
for i in xrange(len(level1_hanzi_index)):
    ...
    level1_hanzi_pairs.append(
        (level1_hanzi_index[i], (hanzi_lead, hanzi_trail)))
...
for row in xrange(0x7E - 0x20):
    for column in xrange(157):
        ...
        if code_point and code_point >= 0x4E00 and code_point <= 0x9FCB:
            ...
            if code_point == 0x5341 or code_point == 0x5345 or not big5_unified_ideograph_bytes[
                    unified_offset]:
                big5_unified_ideograph_bytes[unified_offset] = (
                    unified_lead, unified_trail)
...
data_file.write(
    '''const IBM_SYMBOL_START: usize = %d;''' %
    (run_start_array_index + 1))
data_file.write(
    '''const IBM_SYMBOL_END: usize = %d;''' %
...
for row in xrange(0x20):
    for column in xrange(190):
        i = column + (row * 190)
        # Skip the gaps
        if (column >= 0x1A and column < 0x20) or (
                column >= 0x3A and column < 0x40):
            continue
        code_point = index[i]
...
write_variant_method(
    "max_utf16_buffer_length", False, [
        ("byte_length", "usize")], "Option<usize>", decoder_variants, [], "Decoder")

write_variant_method("max_utf8_buffer_length_without_replacement", False, [
                     ("byte_length", "usize")], "Option<usize>", decoder_variants, [], "Decoder")

write_variant_method(
    "max_utf8_buffer_length", False, [
        ("byte_length", "usize")], "Option<usize>", decoder_variants, [], "Decoder")

write_variant_method("decode_to_utf16_raw",
                     True,
                     [("src",
                       "&[u8]"),
                      ("dst",
...
write_variant_method("decode_to_utf8_raw",
                     True,
                     [("src",
                       "&[u8]"),
                      ("dst",
...
write_variant_method("max_buffer_length_from_utf16_without_replacement", False, [
                     ("u16_length", "usize")], "Option<usize>", encoder_variants, [], "Encoder")

write_variant_method("max_buffer_length_from_utf8_without_replacement", False, [
                     ("byte_length", "usize")], "Option<usize>", encoder_variants, [], "Encoder")

write_variant_method("encode_from_utf16_raw",
                     True,
                     [("src",
                       "&[u16]"),
                      ("dst",
...
write_variant_method("encode_from_utf8_raw",
                     True,
                     [("src",
                       "&str"),
                      ("dst",
...
for pointer in range(0, 94 * 94):
    ...
    if code_point:
        ...
        if revised_pointer == 8644 or (
                revised_pointer >= 1207 and revised_pointer < 1220):
...
for pointer in range(0, len(index)):
    code_point = 0xE000 - 8836 + \
        pointer if pointer >= 8836 and pointer <= 10715 else index[pointer]
    if code_point:
...
for pointer in range(0, 94 * 94):
    ...
    if code_point:
        ...
        lead += 0x21
        trail += 0x21
        iso_2022_jp_out_ref_file.write(
            "\x1B$B%s%s\x1B(B\n" %
            (chr(lead), chr(trail)))
        iso_2022_jp_out_file.write(
            (u"%s\n" % unichr(code_point)).encode("utf-8"))
for i in xrange(len(half_width_index)):
    code_point = i + 0xFF61
...
for pointer in range(((0xA1 - 0x81) * 157), len(index)):
    ...
    if code_point:
        if code_point in prefer_last:
            if pointer != pointer_for_prefer_last[prefer_last.index(
                    code_point)]:
                continue
        else:
...
(codepage_test_begin, codepage_test_end) = read_non_generated(
    "../codepage/src/tests.rs")
...
for code_page in code_pages:
    codepage_test_file.write(
        "    assert_eq!(to_encoding(%d), Some(%s));\n" %
        (code_page, to_constant_name(
            encodings_by_code_page[code_page])))
...
for name in preferred:
    if code_pages_by_encoding.has_key(name):
        codepage_test_file.write(
            "    assert_eq!(from_encoding(%s), Some(%d));\n" %
            (to_constant_name(name), code_pages_by_encoding[name]))
    else:
        codepage_test_file.write(
            "    assert_eq!(from_encoding(%s), None);\n" %
            to_constant_name(name))
