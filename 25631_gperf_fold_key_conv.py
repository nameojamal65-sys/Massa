...
REG_HASH_FUNC = re.compile(
    'hash\\s*\\(register\\s+const\\s+char\\s*\\*\\s*str,\\s*register\\s+size_t\\s+len\\s*\\)')
REG_STR_AT = re.compile('str\\[(\\d+)\\]')
REG_RETURN_TYPE = re.compile('^const\\s+short\\s+int\\s*\\*')
REG_FOLD_KEY = re.compile(
    'unicode_fold(\\d)_key\\s*\\(register\\s+const\\s+char\\s*\\*\\s*str,\\s*register\\s+size_t\\s+len\\)')
REG_ENTRY = re.compile('\\{".*?",\\s*(-?\\d+)\\s*\\}')
REG_IF_LEN = re.compile('\\s*if\\s*\\(\\s*len\\s*<=\\s*MAX_WORD_LENGTH.+')
REG_GET_HASH = re.compile(
    '(?:register\\s+)?(?:unsigned\\s+)?int\\s+key\\s*=\\s*hash\\s*\\(str,\\s*len\\);')
REG_GET_CODE = re.compile(
    '(?:register\\s+)?const\\s+char\\s*\\*\\s*s\\s*=\\s*wordlist\\[key\\]\\.name;')
REG_CODE_CHECK = re.compile(
    'if\\s*\\(\\*str\\s*==\\s*\\*s\\s*&&\\s*!strncmp.+\\)')
...
def parse_line(s, key_len):
    ...
    r = re.sub(
        REG_CODE_CHECK,
        'if (index >= 0 && onig_codes_cmp(codes, OnigUnicodeFolds%d + index, %d) == 0)' %
        (key_len,
         key_len),
        s)
    if r != s:
