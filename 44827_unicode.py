...
class UnicodeData(object):
    def __init__(self):
        ...
        def stats(name, table):
            ...
            print(
                "%s: %d chars => %d decomposed chars" %
                (name, len(table), count))
    ...
    def _load_unicode_data(self):
        ...
        for line in self._fetch("UnicodeData.txt").splitlines():
            ...
            if decomp.startswith('<'):
                self.compat_decomp[char_int] = [
                    int(c, 16) for c in decomp.split()[1:]]
            elif decomp != '':
                self.canon_decomp[char_int] = [
                    int(c, 16) for c in decomp.split()]
    ...
    def _load_norm_tests(self):
        ...
        for line in self._fetch("NormalizationTest.txt").splitlines():
            ...
            source, nfc, nfd, nfkc, nfkd = [
                [c.strip() for c in p.split()] for p in test_pieces[:5]]
        ...
        def _decompose(char_int, compatible):
            ...
            if decomp is not None:
                for decomposed_ch in decomp:
                    for fully_decomposed_ch in _decompose(
                            decomposed_ch, compatible):
    ...
    def _compute_stream_safe_tables(self):
        ...
        for c in set(self.canon_fully_decomp) | set(self.compat_fully_decomp):
            decomposed = self.compat_fully_decomp.get(
                c) or self.canon_fully_decomp[c]
...
def hexify(c): return '{:04X}'.format(c)
...
""")
    ...
    def str_literal(s): return '"%s"' % "".join("\\u{%s}" % c for c in s)
...
# Compute minimal perfect hash function, d can be either a dict or list of
# keys.
...
if __name__ == '__main__':
    ...
    with open("tables.rs", "w", newline="\n") as out:
        ...
        out.write(
            "pub const UNICODE_VERSION: (u64, u64, u64) = %s;\n\n" %
            version)
        ...
        gen_decomposition_tables(
            data.canon_fully_decomp,
            data.compat_fully_decomp,
            out)
