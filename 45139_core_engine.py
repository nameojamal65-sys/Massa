...
class SovereignCoreEngine:
    ...
    def handle(self, command: str, token: str, signature: str | None = None):
        ...
        payload_str = json.dumps({"tenant": tenant,
                                  "type": task_type,
                                  "prompt": prompt},
                                 ensure_ascii=False,
                                 sort_keys=True)
        if signature is None or not verify(payload_str, signature):
            return {
                ...
                "hint": "use CLI/SDK which signs requests"}
        ...
        if task_type == "video" and diffusion_requires_admin():
            if ("diffusion" in prompt.lower()
                    or "cinematic" in prompt.lower() or "سينمائي" in prompt):
                if role != "admin":
                    return {
                        "status": "blocked",
                        "reason": "diffusion_requires_admin"}

        # 4) Quota (per-tenant plan)
        # Determine plan from identity tenants table if exists; default free
        plan = "free"
        ok, meta = check(tenant, plan, kind=("gpu" if ("diffusion" in prompt.lower(
        ) or "cinematic" in prompt.lower() or "سينمائي" in prompt) else "job"))
        if not ok:
            return {
                "status": "blocked",
        ...
        if not require_human_approval(task, role=role):
            append({"type": "task_blocked_hitl", "user": user,
                   "role": role, "tenant": tenant, "task": task})
            return {"status": "blocked", "reason": "HITL"}

        # 6) Accept
        tid = self.tm.create(task)
        append({"type": "task_accepted", "id": tid, "user": user,
               "role": role, "tenant": tenant, "task": task})
