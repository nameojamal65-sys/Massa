...
def load_property(filename: str, pattern: str, action: Callable[[int], None]):
    with fetch_open(filename) as properties:
        ...
        multiple = re.compile(
            rf"^([0-9A-F]+)\.\.([0-9A-F]+)\s*;\s*{pattern}\s+")
...
def to_sorted_ranges(iter: Iterable[Codepoint]
...
assert len(set([v.value for v in WidthState])) == len(
    [v.value for v in WidthState])
...
def load_east_asian_widths() -> list[EastAsianWidth]:
    ...
    with fetch_open("EastAsianWidth.txt") as eaw:
        ...
        # matches a width assignment for a range of codepoints, i.e.
        # "3001..3003;W  # ..."
        multiple = re.compile(
            r"^([0-9A-F]+)\.\.([0-9A-F]+)\s*;\s*(\w+) +# (\w+)")
        ...
        for line in eaw.readlines():
            ...
            while current <= high:
                ...
                # (resolves to narrow)
                width_map.append(
                    EastAsianWidth.NARROW if current < low else width)
                current += 1

        while len(width_map) < NUM_CODEPOINTS:
            # Catch any leftover codepoints and assign them implicit
            # Neutral/narrow width.
...
def load_ligature_transparent() -> list[tuple[Codepoint, Codepoint]]:
    ...
    default_ignorable_combinings = default_ignorables.intersection(
        combining_marks)
...
def load_solidus_transparent(
    ...
    return list(
        filter(
            lambda range: range not in ligature_transparents,
            sorted))
...
class Table:
    ...
    def indices_to_widths(self):
        ...
        """
        self.entries = list(
            map(lambda i: int(self.indexed[i].width()), self.entries))  # type: ignore
        del self.indexed
...
def load_text_presentation_sequences() -> list[Codepoint]:
    ...
    codepoints = []
    for cp in text_presentation_seq_codepoints.intersection(
...
"""
        ...
        for msbs, i in text_presentation_idx:
            module.write(
                f"        0x{
                    msbs:X} => &TEXT_PRESENTATION_LEAF_{i},\n")
            ...
            if table.bytes_per_row is None:
                ...
                module.write(
                    f"static {
                        table.name}: Align{
                        table.align}<[u8; {
                        len(byte_array)}]> = Align{
                        table.align}([")
                for j, byte in enumerate(byte_array):
                    # Add line breaks for every 15th entry (chosen to match
                    # what rustfmt does)
                    if j % 16 == 0:
                        module.write("\n   ")
...
def main(module_path: str):
    ...
    print(
        f"Generating module for Unicode {
            version[0]}.{
            version[1]}.{
                version[2]}")
    ...
    solidus_transparent = load_solidus_transparent(
        ligature_transparent, cjk_width_map)
    ...
    for s, table in [
        ...
        index_size = len(table[0]) * \
            (math.ceil(math.log(table[0][-1][0], 256)) + 8)
    ...
    for s, table in [
        ...
        index_size = len(table[0]) * \
            (math.ceil(math.log(table[0][-1][0], 256)) + 16)
        print(f"{s} index size: {index_size} bytes")
        total_size += index_size
