...
if [ ! -d ".venv" ]; then
  echo "❌ .venv غير موجودة. أنشئ البيئة أولاً."
  exit 1
fi
...
def ui_logout():
    # logout الفعلي يتم من JS (مسح التوكن). هذا فقط Redirect.
...
<head>
  ...
  <style>
    ...
    header { padding: 16px 20px; background:#0f1621; border-bottom:1px solid #1c2a3a; display:flex; align-items:center; justify-content:space-between; gap: 12px; flex-wrap: wrap; }
    .brand { font-weight:800; letter-spacing:.3px; }
    .container { padding: 18px 20px; max-width: 1100px; margin: 0 auto; }
    .card { background:#0f1621; border:1px solid #1c2a3a; border-radius: 14px; padding: 16px; margin-bottom: 14px; box-shadow: 0 10px 30px rgba(0,0,0,.18); }
    ...
    button { cursor:pointer; background:#1b3b63; border-color:#2a568f; font-weight:800; }
    ...
    th, td { border-bottom: 1px solid #1c2a3a; padding: 10px; text-align: right; font-size: 14px; vertical-align: top; }
    th { color:#9fb2c9; font-weight: 800; }
...
<div class="card">
  ...
  <div class="muted">الواجهة تحفظ Access Token في المتصفح (localStorage) ثم تستخدمه للـ API.</div>
</div>
...
<div class="card">
  ...
  <div class="muted">
    لو ما عملت bootstrap بعد: استخدم <code>/docs</code> أو endpoint:
